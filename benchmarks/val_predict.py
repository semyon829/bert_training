...
@torch.no_grad()
@hydra.main(config_path="../cfg/benchmark", config_name="val_predict")
def test_predict(cfg: DictConfig):
    ...
    test_dataset = loader(cfg.dataset, dataset_type="val", train=False, tokenizer=tokenizer, transforms=test_transforms)
    ...
    for data_test in tqdm(test_loader):
        ...
        input_test, label_test = data_test["input"], data_test["label"]
    ...
    meta["pred"] = test_all_outputs

    save_path = os.path.join(
        os.path.dirname(os.path.dirname(cfg.model_weights_path)), 
        f"val_predict_{os.path.basename(os.path.splitext(cfg.model_weights_path)[0])}.csv"
    )
